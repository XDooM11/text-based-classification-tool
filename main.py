...
class Auditor:
    ...
    def __init__(self, target_dir):
        ...
        self.fakulty_data = {
            "Cyrilometodejska teologicka fakulta":
                r"Cyrilometodejsk[ae]\s*teologick[ae]\s*fakulta|Teologick[ae]\s*fakulta|Univerzitni\s*22|\bcmtf\b",

            "Lekarska fakulta":
                r"Lekarsk[ae]\s*fakulta|Hnevotinska\s*3|\blf\b",

            "Filozoficka fakulta":
                r"Filozofick[ae]\s*fakulta|Krizkovskeho\s*10|Krizkovskeho\s*14|Vodarni\s*6|tr\.\s*Svobody\s*26|\bff\b",

            "Prirodovedecka fakulta":
                r"Prirodovedeck[ae]\s*fakulta|17\.\s*listopadu\s*50a?|Slechtitelu\s*27|Slechtitelu\s*17|Slechtitelu|\bprf\b|Prirodoveda|botanick[ae]\s*zahrad[ay]|U\s*botanicke\s*zahrady\s*920|1192\/12",

            "Pedagogicka fakulta":
                r"Pedagogick[ae]\s*fakulta|Zizkovo\s*nam|Purkrabska\s*2|\bpdf\b",

            "Pravnicka fakulta":
                r"Pravnick[ae]\s*fakulta|17\.\s*listopadu\s*(6|8)|\bpf\b",
    ...
    def run_audit(self):
        ...
        for i, file in enumerate(files, 1):
            ...
            print(f"[{i}/{total}] Zpracovavam: {file.name}")
    ...
    def export_pdf(self, output_name="Report1.pdf"):

        print("Generuji PDF report...")
        ...
        pdf.cell(10, 15, "")
        pdf.cell(90, 15, f"Z toho k auditu: {len(self.results) + len(self.unmatched)}",
                 border=1, fill=True, align='C', new_x="LMARGIN", new_y="NEXT")

        pdf.ln(10)

        pdf.set_font("Helvetica", "I", 10)
        pdf.cell(0, 8, f"Celkovy objem dat: {df['size'].sum():.2f} MB",
                 new_x="LMARGIN", new_y="NEXT")

        pdf.ln(5)

        pdf.set_font("Helvetica", "B", 14)
        pdf.cell(0, 10, "1. Rozdeleni dle typu souboru",
                 new_x="LMARGIN", new_y="NEXT")
        ...
        for _, row in df_summary.iterrows():
            ...
            pdf.cell(50, 8, f"{row['Velikost_MB']:.2f}", 1,
                     new_x="LMARGIN", new_y="NEXT")

        pdf.add_page()

        pdf.set_font("Helvetica", "B", 14)
        pdf.cell(0, 10, f"SEZNAM ZARAZENYCH ({len(self.results)})",
        ...
        pdf.set_font("Helvetica", "B", 14)
        pdf.cell(0, 10, f"SEZNAM NEZARAZENYCH ({len(self.unmatched)})",
                 align='C', new_x="LMARGIN", new_y="NEXT")
